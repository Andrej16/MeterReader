...
static void RegisterServices(WebApplicationBuilder webAppBuilder)
{
    webAppBuilder.Services.AddScoped<JwtTokenValidationService>();
    webAppBuilder.Services.AddAuthentication()
      .AddJwtBearer(cfg =>
      {
          cfg.TokenValidationParameters = new MeterReaderTokenValidationParameters(webAppBuilder.Configuration);
      });

    webAppBuilder.Services.AddCors(cfg =>
    {
    ...
    webAppBuilder.Services.AddDbContext<ReadingContext>(options =>
        options.UseSqlServer(connectionString));
    webAppBuilder.Services.AddDatabaseDeveloperPageExceptionFilter();

    webAppBuilder.Services.AddDefaultIdentity<IdentityUser>(options => options.SignIn.RequireConfirmedAccount = false)
        .AddEntityFrameworkStores<ReadingContext>();

    webAppBuilder.Services.AddScoped<IReadingRepository, ReadingRepository>();

    webAppBuilder.Services.AddRazorPages();

    webAppBuilder.Services.AddGrpc(cfg => cfg.EnableDetailedErrors = true);
}
